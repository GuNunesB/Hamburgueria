...
const lancheModel = require('./src/models/Lanche.js')
...
const buscarLanche = async (nome) => {
    try {
        ...
        if (nomesLanches.length === 0) {
            ...
            console.log(`Nome: ${melhorLanche.nomeLanche}, Descrição: ${melhorLanche.descricaoLanche}, Preço: ${melhorLanche.precoLanche}, img: ${melhorLanche.imagemLanche}`)
        }
    } catch (error) {
        console.log(error)
    }
...
const deletarLanche = async (id) => {
    try {
        const lanche = await lancheModel.findByIdAndDelete
...
const app = async () => {
    ...
    // READ - await listarLanches(), await buscarLanche("Mari")
    // UPDATE - await atualizarLanche('67b904ed21f973a37731aae4', 'Mario Lúcio', '99999-0012', '909.090.909-67')
    // DELETE - await deletarLanche('67b904ed21f973a37731aaed')
    await desconectar()
